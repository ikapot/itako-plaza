        ...
        {isSpectral ? (
          ...
          >
            {char.avatar ? (
              ...
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold mb-1 border border-white/10 ${char.color || 'bg-zinc-800/80'} text-white font-biz-mincho`}>
                {char.name.slice(0, 1)}
              </div>
            )}
            <span className={`text-[8px] font-biz-mincho font-bold uppercase tracking-wider text-center leading-tight px-1 text-white`}>
              {char.name}
            </span>
    ...
    >
      <AnimatePresence mode="wait">
        {isSpectral ? (
          ...
          >
            <span className={`text-[9px] font-biz-mincho font-bold uppercase tracking-widest text-center leading-tight text-white`}>
              {loc.name}
            </span>
    ...
    <div className="relative w-full select-none space-y-3">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-3 items-center lg:items-start">
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <span className="text-[8px] md:text-[10px] text-white/20 font-biz-mincho">{CHAR_THEME[charFaceIdx]}</span>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <span className="text-[8px] md:text-[10px] text-white/25 font-biz-mincho">{LOC_THEME[locFaceIdx]}</span>
          </div>
      ...
      <div className="mx-0.5 py-3 px-4 bg-white/5 border border-white/10 rounded-xl">
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-[9px] md:text-[11px] text-white/30 uppercase tracking-widest font-biz-mincho shrink-0">召喚：</span>
          {selectedChars.length > 0 ? selectedChars.map(function render(c) {
              return (
                <span key={c.id} className={`text-[9px] md:text-[11px] px-2.5 py-1 rounded border transition-all font-bold font-biz-mincho ${c.isMainChar ? 'text-[#f15a24] bg-[#f15a24]/10 border-[#f15a24]/30' : 'text-white/80 bg-white/5 border-white/10'}`}>
                    {c.name}
                </span>
              );
          }) : <span className="text-[9px] md:text-[11px] text-white/15 italic font-biz-mincho">未選択（最大3人）</span>}
          {selectedLoc ? (
            <>
