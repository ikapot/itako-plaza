...
- **プランニング第一主義 (Planning First Protocol)**: いかなる作業（ファイル操作・コマンド実行）を開始・継続する前であっても、**必ず最初に `PLAN.md`（ルート直下）を作成または更新する。** ここに「現在の意図（目的）」と「これから行う作業ステップ」をチェックボックス形式で記載し、フリーズした際の復旧ポイントとする。
